...
class DatabaseService:
            ...
            if len(name_parts) > 0:
                ...
                for part in name_parts:
                    if len(part) > 2:  # Only consider parts with more than 2 characters
                        name_queries.append({"full_name": {"$regex": f"\\b{part}\\b", "$options": "i"}})
    ...
    async def get_tasks(self, query=None):
        ...
        try:
            ...
            # Execute the query
            tasks = await self.tasks_collection.find(query_filters).sort("start_date", -1).to_list(length=100)
            
            # Log the found tasks for debugging
            if tasks:
                task_info = [f"{task.get('title')} - Date: {task.get('start_date')}" for task in tasks[:5]]
            ...
            for task in tasks:
                if "assigned_to" in task and task["assigned_to"]:
                    user = await self.users_collection.find_one({"_id": task["assigned_to"]}, {"full_name": 1})
                    if user:
    ...
    async def get_activities(self, filters: Dict[str, Any] = None, limit: int = 20) -> List[Dict[str, Any]]:
        """Get activities with optional filters"""
        try:
            query = filters or {}
            activities = await self.activities_collection.find(query).sort("start_time", -1).limit(limit).to_list(length=limit)

            # Enrich activities with creator names
            for activity in activities:
                if "created_by" in activity and activity["created_by"]:
                    try:
                        user = await self.users_collection.find_one({"_id": ObjectId(activity["created_by"])}, {"full_name": 1})
                        if user:
    ...
    async def get_resident_tasks(self, resident_id: str, time_range: Dict[str, datetime] = None) -> List[Dict[str, Any]]:
    ...
    async def get_tasks_by_time_range(self, start_time: datetime, end_time: datetime) -> List[Dict[str, Any]]:
    ...
    async def get_activities_by_time_range(self, start_time: datetime, end_time: datetime) -> List[Dict[str, Any]]:
