...
async def fetch_residents(user_name):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            residents_url = f"{API_BASE_URL}/residents/getAllResidents?caregiver_name={user_name}"
            ...
            async with session.get(residents_url) as response:
                if response.status == 200:
                    ...
                    logger.info(
                        f"Fetched {len(residents)} residents for {user_name}"
                    )
                    return residents
                else:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(medications_url) as response:
                if response.status == 200:
                    ...
                    current_medications = [m for m in medications if m.get("start_date") <= current_date
                                   and m.get("end_date") > current_date]
                    
                    reminder_medications = [m for m in current_medications if m.get("schedule_type") != "custom"]
        ...
        for res in residents:
            try:
                ...
                for med in medications:
                    ...
                    logger.info(f"Processing medication {med.get('medication_name')} for resident {res.get('full_name')}")
                    
                    if med.get("schedule_type") == "day":
                        repeat_interval = med.get("repeat")
                        # check if today is the day for medication
                        ...
                        if remainder == 0:
                            ...
                            await queue_medication_reminder(res, med, chat_id, scheduler)
                    
                    elif med.get("schedule_type") == "week":
                        repeat_interval = med.get("repeat")
                        # check if this week is the week for medication
                ...
                logger.error(f"Error processing medications for resident {res.get('full_name', 'unknown')}: {e}")

    logger.info(
        f"Processed medications reminders for all users"
    )
...
async def queue_medication_reminder(resident, medication, chat_id, scheduler):
    ...
    try:
        for time in medication.get("times_of_day", []):
            ...
            now_sg = datetime.now(sg)
            reminder_time = now_sg.replace(hour=hour, minute=minute, second=0, microsecond=0)
...
async def send_medication_reminder(reminder, chat_id):
    ...
    try:
        ...
        logger.info(
            f"Sent medication reminder: {reminder} to chat {chat_id}"
        )
